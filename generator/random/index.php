...
<main class="main-content">
    <div class="card">
        <div class="input-group">
            <label>最小值</label>
            <input type="number" class="form-control" id="rand-min" value="1">
        </div>
        <div class="input-group">
            <label>最大值</label>
            <input type="number" class="form-control" id="rand-max" value="100">
        </div>
        <div class="input-group">
            <label>生成数量</label>
            <input type="number" class="form-control" id="rand-count" value="1" min="1" max="20">
        </div>
        <button class="btn-primary" onclick="genRandom()">立即生成</button>
        <div class="result-box" id="rand-result">
            <span id="rand-text"></span>
            <button class="copy-btn" onclick="copyText('rand-text')">复制</button>
        </div>
    </div>
</main>
...
function genRandom(){
    ...
    const count=parseInt(document.getElementById('rand-count').value)||1;
    if(min>max){ toast('最小值不能大于最大值'); return; }
    const res=[]; for(let i=0;i<count;i++) res.push(Math.floor(Math.random()*(max-min+1))+min;
    document.getElementById('rand-text').textContent=res.join(', ');
    document.getElementById('rand-result').style.display='flex';
}
</script>
</body>
</html>