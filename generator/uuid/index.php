...
.uuid-list { display:flex; flex-direction:column; gap:8px; margin-top:12px; }
.uuid-item { background:#f8f9fa; border-radius:8px; padding:10px 12px; font-family:monospace; font-size:13px; color:#212529; display:flex; align-items:center; justify-content:space-between; gap:8px; border:1px solid #e9ecef; }
...
<main class="main-content">
    <div class="card">
        <div class="input-group"><label>生成数量</label><input type="number" class="form-control" id="uuid-count" value="5" min="1" max="50"></div>
        <button class="btn-primary" onclick="genUUID()">生成 UUID</button>
...
function genUUID(){
    ...
    for(let i=0;i<count;i++){
        ...
        div.innerHTML='<span>'+uuid+'</span><button class="copy-btn" onclick="copyText(\''+uuid+'\')">复制</button>';
        list.appendChild(div);
    }
}
window.addEventListener('DOMContentLoaded', genUUID);
</script>
</body>
</html>